  ...
  <div class="box box-solid box-info">
    ...
    <div class="box-body">
      <?php $form = ActiveForm::begin([
        'action' =>
        [
          'tugas/updatedatakolektif?group_id='.$group
        ],
        'id' => 'kolektif_post',
        'method' => 'post',
      ]); ?>
      ...
      <?php ActiveForm::end(); ?>
        <button class="btn btn-success" id="ubah">Ubah Data</button>
    </div>
</div>
<input type="hidden" id="idpegawaihidden"
  value=
  <?php
  $jum=count($pegawaicheck);
  for($i=0;$i<$jum;$i++)
  {
    echo $pegawaicheck[$i]['id_pegawai'];
    if($i!=($jum-1))
    {
      echo ',';
    }
...
>
  ...
  function blokabsenshow(){
    ...
    $("document").ready(function(){
      var stridpegawai=$("#idpegawaihidden").val();
      var arridpegawai=stridpegawai.split(",");
      var arridpegawainew=[];
      $("#tugas-id_pegawai").find("input").each(function(i,elem){
        var el=$(this).val();
        if(jQuery.inArray(el,arridpegawai) != -1)
        {
      ...
      $("#ubah").on("click",function(){
        ...
        $("#tugas-id_pegawai").find("input").each(function(i,elem){
          if($(this).is(":checked")){
            arridpegawainew.push($(this).val());
          }
        });
        var idadd=[];var iddel=[];
        for(var i=0;i<arridpegawainew.length;i++)
        {
          if(jQuery.inArray(arridpegawainew[i],arridpegawai)==-1)
          {
            idadd.push(arridpegawainew[i]);
          }
        }
        var $form = $("#kolektif_post");
        ...
        if(idadd.length!=0)
        {
            $form.attr("action",action+"&idadd="+idadd.toString());
        }
